        ...
        return (
          <div className="cartItem flex flex-col pt-2 space-y-2" key={item.id}>
            <div className="cartItem__count flex flex-row justify-between">
              ...
              <button
              className='text-lg px-3 w-fit bg-blue-400 text-white rounded-md hover:bg-blue-500' onClick={() => handleIncrease(item)} 
              >+</button>
              <p className='mx-3 md:mx-4'>{item.count}</p>
              <button className='text-lg px-3 w-fit bg-white text-black rounded-md border-2 border-blue-400 hover:bg-blue-50' onClick={() => {
                if(item.count > 1) {
                  handleDecrease(item);
                } else {
